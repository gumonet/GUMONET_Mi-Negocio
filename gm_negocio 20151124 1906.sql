...
-- Script date 24/11/2015 07:06:40 p. m.
...
CREATE TABLE cotizaciones_detalle (
  ...
  importe_ieps DECIMAL(12, 2) DEFAULT NULL,
  PRIMARY KEY (id_detalle),
...
VIEW v_cotizaciones
AS
	select `c`.`id_cotizacion` AS `id_cotizacion`,`c`.`id_cliente` AS `id_cliente`,`c`.`folio` AS `folio`,`c`.`fecha` AS `fecha`,`c`.`plazo_entrega` AS `plazo_entrega`,`c`.`condicion_pago` AS `condicion_pago`,`c`.`cond_embarque` AS `cond_embarque`,`c`.`subtotal` AS `subtotal`,`c`.`iva` AS `iva`,`c`.`total` AS `total`,`c`.`status` AS `status`,`c`.`forma_pago` AS `forma_pago`,`c`.`metodo_pago` AS `metodo_pago`,`c`.`ieps` AS `ieps`,(case `c`.`status` when '1' then 'pendiente' end) AS `str_status`,(select `clientes`.`nombre` from `clientes` where (`clientes`.`id_cliente` = `c`.`id_cliente`)) AS `cliente` from `cotizaciones` `c`;
...
(1, '', NULL, '2015-11-24 18:14:49', '', '', '', 0.00000, 0.00000, 0.00000, 0, NULL, NULL, NULL),
(2, '', NULL, '2015-11-24 18:31:33', '', '', '', 0.00000, 0.00000, 0.00000, 0, '', '', '0'),
(3, '', NULL, '2015-11-24 18:34:12', '', '', '', 0.00000, 0.00000, 0.00000, 0, '', '', '0');
